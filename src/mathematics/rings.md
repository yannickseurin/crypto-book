...
The additive identity is usually denoted 0, the additive inverse of $a$ is usually denoted $-a$, and the unity is usually denoted 1.
...
If $\AA$ is unitary, then note that
\[
 [k]a = 1a + \cdots + 1a = (1+ \cdots +1)a = ([k]1) a.
\]
Hence, when $\AA$ has a unity, one can safely overload notation and let $ka$ denote both scalar multiplication and ring multiplication, with the convention that $k$ also denotes the ring element $[k]1$.
...
Note that $a^k$ for $k < 0$ cannot be defined unless $a^{-k}$ has a [multiplicative inverse](#units).
...
\]
Moreover, there cannot exists $n' < n$ such that for every $a \in \AA$, $n' a =0$ as this imply $n' 1 = 0$ and would contradict that $n$ is the additive order of 1.
Hence, $\charac(\AA) = n$.
>
...
Finally, that $\AA^*$ is abelian if $\AA$ is commutative is straightforward.
...
In this section, we restrict ourselves to commutative rings.

Let $\AA$ be a commutative ring and $a,b \in \AA$.
We say that $b$ ***divides*** $a$, or that $b$ is a ***divisor*** of $a$, or that $a$ is a ***multiple*** of $b$, denoted $b \divides a$, if there exists $q \in \AA$ such that $a=qb$.
...
It is called a ***greatest common divisor*** (GCD) if it is a common divisor and every common divisor $d'$ of $a$ and $b$ divides $d$ (i.e., it is a greatest element among common divisors for the partial order defined by the divisibility relation).
